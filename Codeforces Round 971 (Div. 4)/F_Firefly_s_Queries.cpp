    ...
    while (t--)
    {
        ...
        while (q--)
        {
            ...
            if (p == q)
            {
                ll middle = (p + 1) * n - p;
                if (r >= middle)
                    sum += query(1, 1, n, max(middle, l) - middle + 1, r - middle + 1);
                if (l < middle)
                    sum += query(1, 1, n, n - middle + l + 1, min(n, n - middle + r + 1));
                cout << sum << nl;
                continue;
            }
            tmp = q - p - 1;
            ...
            sum *= tmp;
            ll lomid = (p + 1) * n - p;
            sum += query(1, 1, n, max(lomid, l) - lomid + 1, (p + 1) * n - lomid);
            sum += query(1, 1, n, n - lomid + l + 1, n);
            ll himid = (q + 1) * n - q;
            sum += query(1, 1, n, 1, r - himid + 1);
            sum += query(1, 1, n, q + 1, q + min(r - q * n + 1, n - q));
            cout << sum << nl;
        }
    }
    return SH;
